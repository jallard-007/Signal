...
TEST_CASE("read line and store on stack", "[interpreter]") {
  ...
  (bc)OpCode::ADD_I, 2, 8, 0, 0, 0,
  (bc)OpCode::LOAD_Q, 11, 2, // load stdin FILE * from data
  (bc)OpCode::NOP,
  (bc)OpCode::MOVE_I, 12, '\n', 0, 0, 0, // set j to newline char
  (bc)OpCode::NOP, (bc)OpCode::NOP,
  (bc)OpCode::SUB_I, stackPointerIndex, 32, 0, 0, 0, // make room on stack for array of 32 chars
  (bc)OpCode::MOVE, 13, stackPointerIndex, // set i to start of buffer

  // loop here
  (bc)OpCode::SUB_I, stackPointerIndex, 8, 0, 0, 0, // make room for return value (int) + padding (4 bytes)
  ...
  (bc)OpCode::RS_JUMP_NE, (bc)(-20), // loop if its not newline
  ...
  REQUIRE(program[6] == (bc)OpCode::ADD_I);
...
TEST_CASE("jump", "[interpreter]") {
  ...
  bc const program[] = {
    ...
    // index 26:
    (bc)OpCode::RS_JUMP_E, 8, // jumps to exit
    (bc)OpCode::MOVE_I, 3, 1, 0, 0, 0, // set exit reg to 1, this is how the test case is checked
    // index 50:
    (bc)OpCode::EXIT, 3
  };
  REQUIRE(program[6] == (bc)OpCode::MOVE_I);
  Interpreter interpreter(program, data, 128);
